...
# This file was *autogenerated* from the file fastSolution.sage
from sage.all_cmdline import *   # import sage library

_sage_const_7514777789 = Integer(7514777789); _sage_const_2 = Integer(2); _sage_const_0 = Integer(0); _sage_const_1 = Integer(1)
...
def compTree(X):
    ...
    while (len(nodes) > _sage_const_2 ):
        new_nodes = []
        for j in range(_sage_const_0 , len(nodes)-_sage_const_1 , _sage_const_2 ):
            node = nodes[j] * nodes[j+_sage_const_1 ]
            new_nodes.append(node)
        if len(nodes) % _sage_const_2  == _sage_const_1 :
            new_nodes.append(nodes[-_sage_const_1 ]) #If we had Odd nodes then we need to just throw in that left over leaf for next computation
        nodes = new_nodes

    return nodes[_sage_const_0 ] * nodes[_sage_const_1 ] # This is the root
...
def fastEval(f, tree):
    if f.degree() < _sage_const_2  or tree.poly.degree() < _sage_const_2 :
        if tree.poly.degree() == _sage_const_1 :
            return [f(-tree.poly(_sage_const_0 ))] # Case where we use our simplified poly R(X) for a given Xi
        if f.degree() == _sage_const_0 :
...
def _fast_interpolate(weights, tree):
    if len(tree) == _sage_const_1 :
        return weights[_sage_const_0 ]
...
open("output.bmp", "wb").write(bytearray(y.coefficients(sparse=False)[:-_sage_const_1 ]))
